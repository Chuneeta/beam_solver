...
beam_solver_dir = os.path.dirname(os.path.realpath(__file__))
version_file = os.path.join(beam_solver_dir, 'VERSION')
...
def construct_version_info():
    ...
    try:
        git_origin = subprocess.check_output(['git', '-C', beam_solver_dir, 'config',
                                              '--get', 'remote.origin.url'],
                                             stderr=subprocess.STDOUT).strip()
        git_hash = subprocess.check_output(['git', '-C', beam_solver_dir, 'rev-parse', 'HEAD'],
                                           stderr=subprocess.STDOUT).strip()
        git_description = subprocess.check_output(['git', '-C', beam_solver_dir,
                                                   'describe', '--dirty', '--tag', '--always']).strip()
        git_branch = subprocess.check_output(['git', '-C', beam_solver_dir, 'rev-parse',
                                              '--abbrev-ref', 'HEAD'],
                                             stderr=subprocess.STDOUT).strip()
        git_version = subprocess.check_output(['git', '-C', beam_solver_dir, 'describe', '--always',
                                               '--tags', '--abbrev=0']).strip()
    except:  # pragma: no cover  - can't figure out how to test exception.
        try:
            # Check if a GIT_INFO file was created when installing package
            git_file = os.path.join(beam_solver_dir, 'GIT_INFO')
...
beam_solver_version_str = ('beam_solver version: ' + version + '.')
if git_hash is not '':
    beam_solver_version_str += ('  Git origin: ' + str(git_origin) +
