...
def get_flux(fitsname, ra, dec, plot=False):
	...
	select = R < 2 * bm_radius_px
        ...
        rms = np.sqrt(np.nanmean(imdata[~select])**2) # calculated rm outside the selected region
        ...
        try:
            ...
            peakval, minval, rms, gauss_peak, gauss_int = np.nan, np.nan, np.nan, np.nan, np.nan                
    return {'freq':freq, 'peak':peakval, 'min':minval, 'rms':rms, 'gauss_peak':gauss_peak, 'gauss_int':gauss_int}
