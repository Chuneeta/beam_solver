...
class LinearSolverNoise(LinearSolver):

    def __init__(self, data, noise, wgts={}, sparse=False, **kwargs):
        LinearSolver.__init__(self, data, wgts, sparse, **kwargs)
        self.noise = noise

    def _gen_AtAiAt(self, A, rcond):
        '''Helper function for forming (At A)^-1 At.  Uses pinv to invert.'''
        At = A.T.conj()
        AtN = At.dot(self.noise)
        AtNA = AtN.dot(A)
        # finding inverse is about 3x slower than solve
        try: AtNAi = np.linalg.pinv(AtNA, rcond=rcond)
        except(np.linalg.LinAlgError):
            AtNAi = np.linalg.inv(AtNA)
        AtNAiAtN = AtNAi.dot(AtN)
        return AtNAiAtN
    ...
    def _invert_lsqr(self, A, y, rcond):
        ...
        there are a couple corner cases where lstsq is unstable but pinv works
        for the same rcond. It seems particularly the case for single precision matrices.'''
        AN = A.T.conj().dot(self.noise)
        xhat = np.linalg.lstsq(AN, y[...,0], rcond=rcond)[0]
        xhat.shape += (1,)
        return xhat
    ...
    def _invert_solve(self, A, y, rcond):
        ...
        AtNA = AtN.dot(A)
        AtNy = AtN.dot(y)
        return np.linalg.solve(AtNA, AtNy) # is supposed to error if singular, but doesn't seem to.
